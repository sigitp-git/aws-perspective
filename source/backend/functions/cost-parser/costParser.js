  ...
  if (results.Items && results.Items.length > 0) {
    for (let data of results.Items) {
      ...
      if (newLineItems.length === 0) {
        deleteJobs.push(
          retry(this, deleteDynamo, [key], 5, 200, "deleteDynamo")
        );
        deleteCounter++;
      } else {
        if (update) {
    ...
    await new Promise((resolve, reject) => {
      ...
      readlineStream.on("line", function (line) {
        if (line.indexOf("identity/Line") === -1) {
          allPromises.push(
            processLine(handleLineData, line, resourceProcessor)
          );
        }
      });
...
const handleLineData = (line) => {
  ...
  addKeyToObject(
    data,
    "resourceDayKey",
    hashKey(columns[16] + "-" + getDayOfYear(timeInterval[0]))
  );
...
const dumpError = (err, metaData) => {
  ...
  if (typeof err === "object") {
    ignoredErrors.includes(err.message)
      ? outputSmallError(err, metaData)
      : outputFullError(err, metaData);
  } else {
